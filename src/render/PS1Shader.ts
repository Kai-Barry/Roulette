...
export const PS1Shader = {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // 2. Dithering: Apply subtle Bayer matrix dither noise
      vec2 pixelPos = uv * uResolution;
      float ditherStrength = 0.012; // softened noise so text & fine details stay clear
      float dither = getDither(pixelPos) * ditherStrength;
      col += vec3(dither);
      
      // 3. Color Depth Reduction: Banding to 128 levels (high precision color preservation)
      float colorSteps = 128.0;
